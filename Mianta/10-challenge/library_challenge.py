...
# It is a string list due to brackets and comma's it is defined = has a string to list the books. This can be check by checking type print(type(books))
print()
print(type(books))
print()
#1.1
...
#Return: Not needed for this function
def check_out(book):
   books.remove(book)#we are removing the book (variable) from the list but anything can go in ()
check_out('SAPIENS') #this has to be in a string. calling the function
#1.4
...
print(books)
print('SAPIENS' in books) #this is to check if the 'book' is in the books list
#1.5
#Create a function 'check_in' that adds a book to the books list
#Parameters: book (string)
#Return: Not needed for this function
def check_in(book):
    books.append(book)#this tells the computer which list we are working on and this one is book
check_in('SAPIENS')# Check in 'SAPIENS' using the check_in function# 1.6
print()
...
def search_by_name(book):# created a function that takes book as a variable 
    if book in books: # We are doing something to the variable book. because you are looking for whether the book in the books list. this is why an if else statement is used
        print('Available')
    else: #there are only 2 options so else is the only option and ':' follows
        print('Not Available')
#1.8
# Search for the book 'JUST MERCY'
search_by_name('JUST MERCY')#this is calling the function
print()
...
# Parameters: Not needed for this function ****NOTE:Parameter are when you are passing an argument so you will know if parameters are needed or not
print()
#Return: number of books (integer)
def count_books():#defining a function *count_books 
    return len(books_with_details)# [use RETURN len] to return the length of books_with_details         
#2.2
# Check the number of books available in the books list using the count_books function
output = count_books()#this is calling the function going to return an integer saving the data to the variable called OUTPUT
print(output)#THIS IS WHERE THE VARIABLE IS PRINTING OUT [OUTPUT]
#2.3
print()
#2.4
# Search for book titles by the author 'Timothy Snyder' using the search_by_author function
def search_by_author (author):#def search_by_author(author) w/ parameters - author (string)
    author_books = []
    for i in books_with_details:#Create a function 'search_by_author' that returns the titles of books by an author
        if author in i(['author']):
            author_books.append(i['title'])# Return - author's books (list of strings)Hint - You will need a for loop, if statement, .append() for this solution!

    print(search_by_author('Timothy Synder'))
