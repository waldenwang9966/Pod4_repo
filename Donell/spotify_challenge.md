...
# Answers To Question 1 & 2 -
 ...
 What changes would you make to the modeling of genre data if we could assign more than one genre to a song? 
 We had a one-to-many relationship between genres and songs. Now that if we are to assign multiple genres to one song, then we have many-to-many relationship between songs and genres.
 So the practice is to create an intermediate table that maps the primary keys in the songs table to the primary keys in the genres table, just like what it is in the Artists Members table.