...
activities =(f"what is your favorite thing to do on a {weekdays}? ")
...
favorite_activities.append(input(f"What is your favorite thing to do on {weekdays}? "))
...
for i in range(len(days)) and range(len(favorite_activities)):
...
for day in days:
    temperature = int((f"Tommorrow is {weekdays}. What's the tempurature today?"))
    if temperature < 50:
        print("Wear a jacket!")
...
birthday_wishes = int(input("how many times would you like to be wished happy birthday?"))
for i in range(birthday_wishes):
