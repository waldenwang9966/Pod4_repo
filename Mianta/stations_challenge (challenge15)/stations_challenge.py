...
class SubwayStation(Station):
        ...
        def show_info(self):
         print(f'{self.station_name} station is located at {self.location}, and {self.lines} are the lines designated stops.')
'''''
station_name: '14th street'
location: '14th street and 7th avenue'
lines: ['1', '2', '3', 'L']
'''
station_name1 = SubwayStation(station_name='14th street', location='14th street and 7th avenue', lines=['1', '2', '3', 'L'])
...
class BusStation(Station):
    def __init__(self, station_name, location, open, close, routes):
        super().__init__(station_name, location)    
        self.routes = routes
        self.open = open
        self.close = close
    ...
    def show_info(self):
        ...
        self.open_station()
        self.close_station()
        print(f'The bus station {self.station_name} is located at {self.location}, the {self.routes} are the routes, and the station is {self.open}')

print('Question 4: Make an example bus station')
'''
Using your BusStation class, instantiate a bus station with the info below. 
Then, run the show_info() method to make sure you get the station_name, location, routes, and whether the station is open printed out. Then, demonstrate that you can close and open the bus station i.e. that the show_info() method reflects correctly when the station is open versus closed
...
busStation1 = BusStation( station_name='NYC Megabus Stop', location='34th street and 12th avenue', routes=['Boston', 'DC', 'Philly'], open=TRUE, close= FALSE)
