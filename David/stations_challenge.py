...
class SubwayStation(Station):
    ...
    def show_info(self):
        print(f'{self.station_name} station is located at {self.location} and {self.lines} lines stops here.')
...
class BusStation(Station):
    ...
    def show_info(self):
        if  self.open == True:
            print(f'{self.station_name} station located at {self.location}  and {self.routes} stops here.\nAnd is open for service!')
        else:
            print(f'Sorry, but {self.station_name} station located at {self.location} is now closed!')
...
'''
our_bus = BusStation("NYC Megabus Stop",  "34th street and 12th avenue", ['Boston', 'DC', 'Philly'] )
our_bus.show_info()
